# solarSim

[![Download](https://img.shields.io/badge/Download-v1.5.11-4C9AFF?labelColor=333333)](#download)
[![Windows](https://img.shields.io/badge/Windows-solarSim.exe-0B1220?labelColor=333333)](https://github.com/Salutatorian/solarSim/releases/download/v1.5.11/solarSim-1.5.11-win-x64.zip)
[![Mac Silicon](https://img.shields.io/badge/Mac-Apple_Silicon-0B1220?labelColor=333333)](https://github.com/Salutatorian/solarSim/releases/download/v1.5.11/solarSim-1.5.11-osx-arm64.dmg)
[![Mac Intel](https://img.shields.io/badge/Mac-Intel-0B1220?labelColor=333333)](https://github.com/Salutatorian/solarSim/releases/download/v1.5.11/solarSim-1.5.11-osx-x64.dmg)
[![License](https://img.shields.io/badge/License-Apache_2.0-3B82F6?labelColor=333333)](LICENSE)

## Download

**[Windows](https://github.com/Salutatorian/solarSim/releases/download/v1.5.11/solarSim-1.5.11-win-x64.zip)** — unzip, then double-click `solarSim.exe`. If Windows blocks it: **More info → Run anyway**.

**[Mac (Apple Silicon)](https://github.com/Salutatorian/solarSim/releases/download/v1.5.11/solarSim-1.5.11-osx-arm64.dmg)** — M1 / M2 / M3 / M4. Open the file, drag to Applications, then right-click → **Open**.

**[Mac (Intel)](https://github.com/Salutatorian/solarSim/releases/download/v1.5.11/solarSim-1.5.11-osx-x64.dmg)** — same steps as Apple Silicon.

Not sure which Mac? Apple menu → **About This Mac**. Chip = Apple → Silicon. Processor = Intel → Intel.

![solarSim](docs/screenshots/demo.gif)
