...
public sealed class SolarProject
{
        ...
        bool recordHistory = true)
    {
        ...
        if (Roofs.HasAnyClosedRoof)
        {
            var placement = RoofGeometry.EvaluatePanelPlacement(
                Roofs, xMm, yMm, definition.WidthMm, definition.HeightMm);
            if (!placement.IsValid)
            {
                var host = Roofs.Roofs.FirstOrDefault(r => r.HasRoof && r.IsVisible) ?? Roof;
                var inset = host.EnforceSetback && host.SetbackMm > 0
                    ? RoofGeometry.InsetConvexPolygon(host.Vertices, host.SetbackMm)
                    : host.Vertices.ToList();
                if (inset.Count >= 3)
                {
                    var cx = inset.Average(v => v.X);
                    var cy = inset.Average(v => v.Y);
                    xMm = cx - definition.WidthMm / 2;
                    yMm = cy - definition.HeightMm / 2;
                }
                else if (host.Vertices.Count > 0)
                {
                    var cx = host.Vertices.Average(v => v.X);
                    var cy = host.Vertices.Average(v => v.Y);
                    xMm = cx - definition.WidthMm / 2;
                    yMm = cy - definition.HeightMm / 2;
                }

                panel.SetPosition(xMm, yMm);
            }
        }
